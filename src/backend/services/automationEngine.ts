...
// In-memory process-local lock set to prevent concurrent burst execution for the same rule
const activeRuleEvaluations = new Set<string>();
...
export async function processAutomationRulesForReadings(
  ...
  for (const reading of readings) {
    ...
    for (const rule of rules) {
      ...
      // Check process-local concurrency lock for active rule evaluation
      if (activeRuleEvaluations.has(rule.id)) {
        console.log(`[AUTOMATION-CONCURRENCY-SKIP] Rule '${rule.name}' (${rule.id}) currently evaluating, skipping concurrent burst.`);
        continue;
      }
      activeRuleEvaluations.add(rule.id);

      try {
        await prisma.$transaction(async (tx) => {
          // Acquire PostgreSQL transaction-scoped advisory lock for this rule across instances
          await acquireAdvisoryLock(tx, rule.id);

          // Rule condition is MET. Re-check NO-SPAM condition against action datastream current state
          const actionDatastream = await tx.datastream.findUnique({
            where: { id: rule.actionDatastreamId },
          });

          if (!actionDatastream) return;

          // Get latest state of action datastream AFTER acquiring the lock
          const latestReading = await tx.sensorData.findFirst({
          ...
          const notif = await createNotification(deviceId, 'AUTOMATION_TRIGGERED', notifMessage, tx);

          // 4. Emit new_notification WebSocket event
          emitNotificationToUser(userId, deviceId, notif);
        });
      } finally {
        activeRuleEvaluations.delete(rule.id);
      }
    }
  }
}
